...
-- 문제 11 전체 인구가 1억 이상인 지역과, 이름을 검색하라. 여기서 1억명은 숫자 0이 8개 
select 지역, 이름
from 국가
group by 지역
...
select *
from 국가
where 지역 in((select 지역 from 국가 where 이름 = '한국'), (select 지역 from 국가 where 이름 = '영국') );
